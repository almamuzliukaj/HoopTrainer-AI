...
the database enforces that only the owner can see or create their workouts, even if there is a bug or exploit in the frontend.
...
This proves RLS is active and correct. There are no data leaks between users.
...
The same ownership model now matters beyond the workout CRUD table. The app also stores:

- AI conversations and messages per user;
- saved AI training plans per user;
- player profile, daily challenge completions, and streak freeze usage in Supabase Auth metadata.

The dashboard now uses those user-specific records to calculate XP, ranks, weekly quests, skill badges, level rewards, current streak, and weekly progress. This makes RLS and user ownership important not only for privacy, but also for accurate progress tracking.
